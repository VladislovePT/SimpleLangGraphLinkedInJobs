...
PROFILE:
The candidate's PROFILE is a JSON object where skills are categorized and then grouped by proficiency level: "Expert", "Proficient", or "Intermediate".
Example: "Programming & Scripting": { "Expert": ["Python"], "Proficient": ["Java"] }
...
<div style="border: 1px solid #ddd; border-radius: 8px; margin: 16px 0; padding: 16px; font-family: sans-serif; line-height: 1.6;">
    ...
    <div>
        ...
        <ul style="margin-top: 4px; padding-left: 20px; margin-bottom: 12px;">
            <!-- Repeat for each matching skill -->
...
3.  **Fit Assessment & Colors:**
    - Calculate the fit score by giving more weight to skills listed as 'Expert'. A job requirement matching an 'Expert' skill should contribute more to the score than one matching an 'Intermediate' skill.
...
4.  **Skills Analysis:** For `<MATCHING_SKILLS_LIST>` and `<MISSING_SKILLS_LIST>`, generate a series of `<li>` items.
    - When a skill matches, **you MUST state its proficiency level** from the profile (Expert, Proficient, or Intermediate). An 'Expert' skill is a stronger match than a 'Proficient' or 'Intermediate' one.
    - **Matching Example:** `<li><strong>Python:</strong> Strong match. The job requires Python, and the profile lists it as an **Expert** skill.</li>`
